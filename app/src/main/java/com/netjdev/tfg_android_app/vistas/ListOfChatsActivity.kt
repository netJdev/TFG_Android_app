...
class ListOfChatsActivity : AppCompatActivity() {
    ...
    private fun chatSelectel(chat: Chat) {
    ...
    private fun newChat(){
        ...
        firestore.collection("users").document(user_email).collection("chats").document(chatId).set(chat)
        firestore.collection("users").document(otherUser).collection("chats").document(chatId).set(chat)
